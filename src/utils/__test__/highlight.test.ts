...
test('单一字符串关键字高亮', () => {
  const result = highlight.highlightArea(text, 'a1').result;
  expect(result).toBe('<span style="color: red;">a1</span>b1c1');
});

test('正则表达式关键字高亮', () => {
  expect(highlight.highlightArea(text, /\d/g).result).toBe(
    'a<span style="color: red;">1</span>b<span style="color: red;">1</span>c<span style="color: red;">1</span>',
  );
  expect(highlight.highlightArea(text, /\d/).result).toBe(
    'a<span style="color: red;">1</span>b1c1',
  );
  expect(highlight.highlightArea(text, /A/).result).not.toContain(
    '<span style="color: red;">',
  );
  expect(highlight.highlightArea(text, /A/i).result).toBe(
    '<span style="color: red;">a</span>1b1c1',
  );
});

test('混合关键字高亮', () => {
  const result = highlight.highlightArea(text, ['a', /\d/g]).result;
  expect(result).toBe(
...
test('相同关键字高亮', () => {
  const result = highlight.highlightArea(text, ['a1', 'a1b1']).result;
  expect(result).toBe('<span style="color: red;">a1b1</span>c1');
});

test('词义相同关键字高亮', () => {
  const result = highlight.highlightArea(text, ['a1', 'b1']).result;
  expect(result).toBe('<span style="color: red;">a1b1</span>c1');
});

test('无匹配关键字高亮', () => {
  const result = highlight.highlightArea(text, ['不存在的词']).result;
  expect(result).not.toContain('<span style="color: red;">');
});

test(`严格包裹性测试`, () => {
  const result = highlight.highlightArea(`abcabc`, 'a').result;
  expect(result).toBe(
